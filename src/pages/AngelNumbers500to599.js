...
const AngelNumbers500to599 = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const [activeFilter, setActiveFilter] = useState('all');
    const [showFilters, setShowFilters] = useState(false);
    const navigate = useNavigate();
    // Generate numbers from 500 to 599
    const numbers = Array.from({ length: 100 }, (_, i) => i + 500);
    // SEO-friendly content
    const pageTitle = 'Numerele Angelice de la 500 la 599 | Ghid Complet cu Semnificații';
    const pageDescription = 'Descoperă semnificația numerelor angelice de la 500 la 599. Ghid complet cu interpretări pentru fiecare număr angelic, inclusiv semnificații spirituale, în dragoste și carieră.';
    const pageKeywords = 'numere angelice 500-599, semnificatia numerelor angelice, numere angelice dragoste, numere angelice abundenta, numere angelice spiritualitate';
    // Frequent numbers with their categories and icons
    const frequentNumbers = [
        { number: 555, category: 'Spiritualitate', icon: Sparkles, color: 'purple' },
        { number: 567, category: 'Progres', icon: Target, color: 'blue' },
        { number: 579, category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: 588, category: 'Schimbare', icon: Zap, color: 'yellow' },
        { number: 599, category: 'Dragoste', icon: Heart, color: 'pink' },
        { number: 522, category: 'Înțelepciune', icon: Lightbulb, color: 'indigo' },
        { number: 533, category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: 544, category: 'Completare', icon: Trophy, color: 'amber' }
    ];
    // Popular combinations
    const popularCombinations = [
        { number: '501', category: 'Început Nou', icon: Star, color: 'yellow' },
        { number: '511', category: 'Pace', icon: Shield, color: 'blue' },
        { number: '521', category: 'Creativitate', icon: Sparkles, color: 'purple' },
        { number: '531', category: 'Stabilitate', icon: Target, color: 'green' },
        { number: '541', category: 'Schimbare', icon: Zap, color: 'yellow' },
        { number: '551', category: 'Armonie', icon: Heart, color: 'pink' },
        { number: '561', category: 'Înțelepciune', icon: Lightbulb, color: 'indigo' },
        { number: '571', category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: '581', category: 'Completare', icon: Trophy, color: 'amber' }
    ];
    const handleSearch = (e) => {
        e.preventDefault();
        const num = parseInt(searchQuery);
        if (!isNaN(num) && num >= 500 && num <= 599) {
            navigate(`/semnificatia-numarului-angelic-${num}`);
        }
        else {
            navigate('/numerele-angelice-de-la-500-la-599');
        }
    };
...
export default AngelNumbers500to599;
