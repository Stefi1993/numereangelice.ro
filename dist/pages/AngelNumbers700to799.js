...
const AngelNumbers700to799 = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const [activeFilter, setActiveFilter] = useState('all');
    const [showFilters, setShowFilters] = useState(false);
    const navigate = useNavigate();
    // Generate numbers from 700 to 799
    const numbers = Array.from({ length: 100 }, (_, i) => i + 700);
    // SEO-friendly content
    const pageTitle = 'Numerele Angelice de la 700 la 799 | Ghid Complet cu Semnificații';
    const pageDescription = 'Descoperă semnificația numerelor angelice de la 700 la 799. Ghid complet cu interpretări pentru fiecare număr angelic, inclusiv semnificații spirituale, în dragoste și carieră.';
    const pageKeywords = 'numere angelice 700-799, semnificatia numerelor angelice, numere angelice dragoste, numere angelice abundenta, numere angelice spiritualitate';
    // Frequent numbers with their categories and icons
    const frequentNumbers = [
        { number: 711, category: 'Spiritualitate', icon: Sparkles, color: 'purple' },
        { number: 722, category: 'Progres', icon: Target, color: 'blue' },
        { number: 733, category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: 744, category: 'Schimbare', icon: Zap, color: 'yellow' },
        { number: 755, category: 'Dragoste', icon: Heart, color: 'pink' },
        { number: 766, category: 'Înțelepciune', icon: Lightbulb, color: 'indigo' },
        { number: 777, category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: 788, category: 'Completare', icon: Trophy, color: 'amber' }
    ];
    // Popular combinations
    const popularCombinations = [
        { number: '701', category: 'Început Nou', icon: Star, color: 'yellow' },
        { number: '711', category: 'Pace', icon: Shield, color: 'blue' },
        { number: '721', category: 'Creativitate', icon: Sparkles, color: 'purple' },
        { number: '731', category: 'Stabilitate', icon: Target, color: 'green' },
        { number: '741', category: 'Schimbare', icon: Zap, color: 'yellow' },
        { number: '751', category: 'Armonie', icon: Heart, color: 'pink' },
        { number: '761', category: 'Înțelepciune', icon: Lightbulb, color: 'indigo' },
        { number: '771', category: 'Abundență', icon: DollarSign, color: 'green' },
        { number: '781', category: 'Completare', icon: Trophy, color: 'amber' }
    ];
    const handleSearch = (e) => {
        e.preventDefault();
        const num = parseInt(searchQuery);
        if (!isNaN(num) && num >= 700 && num <= 799) {
            navigate(`/semnificatia-numarului-angelic-${num}`);
        }
        else {
            navigate('/numerele-angelice-de-la-700-la-799');
        }
    };
...
export default AngelNumbers700to799;
